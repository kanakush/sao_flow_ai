...
def get_last_month_range():
    """Вычисляет начало и конец предыдущего месяца"""
    today = datetime.now()
    first_day_this_month = today.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    last_day_last_month = first_day_this_month - timedelta(seconds=1)
    first_day_last_month = (first_day_this_month - timedelta(days=1)).replace(day=1, hour=0, minute=0, second=0)
...
SELECT / here your script
    ...
    try:
        ...
        # --- СОХРАНЕНИЕ ---
        raw_dir = os.getenv("RAW_DATA_PATH", "./raw_month_reports/")
